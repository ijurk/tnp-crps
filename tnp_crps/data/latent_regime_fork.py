...
class LatentRegimeForkGroundTruthPredictor(GroundTruthPredictor):
    ...
    def _base_observation_sample(self, x: torch.Tensor) -> torch.Tensor:
        ...
        cov = cov + (self.noise_std**2 + self.jitter) * eye
    ...
    def sample_outputs(
        ...
        all_offsets = self.branch_offsets(x, self.fork_locations.to(x.device))

        gather_idx = regimes.view(1, batch_size, 1, 1).expand(
            1,
            batch_size,
            x.shape[1],
            1,
        )
        offsets = torch.gather(all_offsets, dim=0, index=gather_idx).squeeze(0)
