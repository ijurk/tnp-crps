"""MLP-SCM prior for synthetic tabular regression.

Adapted from the group's incTNP tabular-data implementation.
"""
...
class CausalMLPPrior:
    def __init__(
        ...
        # Online task generation runs on CPU. GPU generation should only
        # be enabled explicitly, for example in a standalone bank builder.
    ...
    def sample(self, batch_size: int, seq_len: int, *, apply_x_outlier_processing: bool = True):
...
class MLPTabularTaskSource:
    """Independent raw regression-task source based on the MLP-SCM prior.

    Every call to ``sample_task`` samples a new prior mechanism. No
    context/target splitting, normalization, clipping, padding, or
    classification conversion is performed here.
    """
        ...
        *,
        ...
        if self.max_input_features < 1:
            raise ValueError(
                ...
                f"got {self.max_input_features}."
            )

        if self.device != "cpu":
            raise ValueError(
                "Online MLPTabularTaskSource generation must use CPU. "
                "Use a standalone bank-generation process for GPU generation."
            )
    ...
    def _sample_prior_arguments(self) -> dict:
        prior_config = get_default_prior_config(self.device)

        sampled = {
            key: self._sample_value(value)
            for key, value in prior_config.items()
        }

        # V1 scope: continuous numerical regression only.
        sampled["categorical_feature_prob"] = 0.0

        # The wrapper, not the source, owns preprocessing.
        sampled["pre_processing_method"] = "none"
        sampled["outlier_method"] = "none"
    ...
    def sample_task(
        ...
        """Sample one independently parameterized raw tabular task.

        Returns:
            x_raw: [N, D]
            y_raw: [N, 1]
            metadata: task-level numeric metadata
        """
        seq_len = int(seq_len)
        ...
        prior = CausalMLPPrior(
            **self._sample_prior_arguments()
        )

        x_raw, y_raw = prior.sample(
            batch_size=1,
            seq_len=seq_len,
            apply_x_outlier_processing=False,
        )
        ...
        if x_raw.ndim != 2:
            raise RuntimeError(
                f"Expected x_raw [N, D], got {tuple(x_raw.shape)}."
            )

        if y_raw.ndim != 2 or y_raw.shape[-1] != 1:
            raise RuntimeError(
                f"Expected y_raw [N, 1], got {tuple(y_raw.shape)}."
            )

        if x_raw.shape[0] != seq_len or y_raw.shape[0] != seq_len:
            raise RuntimeError(
                "Raw task source returned the wrong sequence length: "
                f"x={tuple(x_raw.shape)}, y={tuple(y_raw.shape)}, "
                f"expected N={seq_len}."
            )
        ...
        if num_features > self.max_input_features:
            raise RuntimeError(
                f"Generated {num_features} features, exceeding "
