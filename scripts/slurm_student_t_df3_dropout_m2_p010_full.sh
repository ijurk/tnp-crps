...
echo "Running Student-t process df=3 Dropout CRPS-TNP training..."
echo "Model: Dropout CRPS-TNP"
echo "Data: Student-t process, df=3"
echo "num_samples M=2, p_dropout=0.10, crps_alpha=1.0"
echo "TNP_RUN_ID=${TNP_RUN_ID}"
...
python -u experiments/lightning_train_crps.py \
  ...
  "$@" \
  --config experiments/configs/generators/student_t_process_1d_df3.yml experiments/configs/models/tnp_crps_dropout.yml
