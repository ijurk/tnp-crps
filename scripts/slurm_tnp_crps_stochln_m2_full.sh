...
echo "Running full CRPS Stochastic LayerNorm TNP M=2 training..."
echo "Official LayerNorm model: from-scratch CRPS training"
echo "train num_samples M=2, val num_samples M=8, layernorm_noise_dim=32, crps_alpha=1.0"
echo "TNP_RUN_ID=${TNP_RUN_ID}"
...
python -u experiments/lightning_train_crps.py \
  ...
  misc.check_val_every_n_epoch=5 \
  misc.checkpoint_interval=50 \
  misc.plot_interval=50 \
  misc.run_group=crps_tnp_layernorm_full \
  "$@" \
  --config external/tnp/experiments/configs/generators/synthetic-1d.yml experiments/configs/models/tnp_crps_stochln.yml
