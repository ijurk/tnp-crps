...
import tnp
import tnp_crps
from tnp_crps.models.tnp_stochln import StochasticLayerNormTNP
from tnp_crps.data.student_t_process import RandomScaleStudentTProcessGenerator
...
print("student-t generator import ok")
PY

echo "Running Student-t process df=3 Stochastic LayerNorm CRPS-TNP training..."
echo "Model: StochLN CRPS-TNP"
echo "Data: Student-t process, df=3"
echo "num_samples M=4, layernorm_noise_dim=32, crps_alpha=1.0"
echo "TNP_RUN_ID=${TNP_RUN_ID}"
...
python -u experiments/lightning_train_crps.py \
  ...
  params.val_num_samples=10 \
  params.test_num_samples=10 \
  misc.logging=True \
  misc.num_workers=1 \
  ...
  misc.run_group=student_t_full \
  "$@" \
  --config experiments/configs/generators/student_t_process_1d_df3.yml experiments/configs/models/tnp_crps_stochln.yml
