...
echo "Running CRPS Stochastic LayerNorm TNP smoke test..."
echo "TNP_RUN_ID=${TNP_RUN_ID}"

python -u experiments/lightning_train_crps.py \
  params.epochs=10 \
  generators.train.samples_per_epoch=4096 \
  generators.val.samples_per_epoch=1024 \
  ...
  misc.checkpoint_interval=5 \
  misc.plot_interval=999 \
  misc.run_group=crps_tnp_layernorm_smoke \
  "$@" \
